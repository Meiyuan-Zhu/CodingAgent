...
**当前阶段：工程骨架、Agent 运行协议领域模型、本地 workspace 边界、读写文件工具、工具注册表、HTTP run API、SSE 事件流、mock runner、模型适配边界、响应解析器、多轮 Agent loop、运行预算、取消 API、工具超时、后台任务生命周期、审批策略、完整 approve/reject/resume 工作流、diff/变更展示和 OpenAI-compatible DeepSeek V4 Flash 适配器已建立；真实 DeepSeek 端到端调用待用户明确授权后验证；尚未实现命令工具。**
...
- OpenAI-compatible 模型适配器已建立：默认仍为 mock，可通过配置切换到 DeepSeek V4 Flash，并从 `DEEPSEEK_API_KEY` 环境变量读取密钥。见 [ADR-0015](../decisions/0015-openai-compatible-deepseek-model-adapter.md)。
...
| 真实模型任务及回归测试 | 阻塞 | [backend/src/main/java/com/zhumeiyuan/codingagent/agent/model/OpenAiCompatibleModelClient.java](../backend/src/main/java/com/zhumeiyuan/codingagent/agent/model/OpenAiCompatibleModelClient.java) | [MODELAPI-001](VERIFICATION.md) | 适配器已通过替身 HTTP 测试；真实 DeepSeek 调用会向外部服务发送 prompt、工具定义和后续工具观察，待用户明确授权后执行 |
| 提交说明、视频与面试材料 | 未开始 | 尚无 | 尚无 | 依据真实能力编写 |

## 下一步

1. 如用户明确授权向 DeepSeek 发送测试 prompt、工具定义和工具观察，执行 DeepSeek V4 Flash 真实端到端 run 验证。
...
- 已实现 DeepSeek/OpenAI-compatible 适配器，但尚未做真实 DeepSeek 端到端连通性验证，不能声称真实模型任务已成功。
- 公开远程仓库尚未建立；当前只有本地 Git 历史。
- 演示案例尚未最终确定，排期属于建议。
- 当前有已验证的 mock run 前后端闭环、写入/编辑工具、多轮 loop、取消、工具超时、审批拦截策略、完整 approve/reject/resume、diff 元数据/展示构建，以及已通过替身 HTTP 测试的 OpenAI-compatible 模型适配器；但没有命令执行、进程级取消或真实 DeepSeek 端到端验证。
