...
- OpenAI-compatible 模型适配器已建立：默认仍为 mock，可通过配置切换到 DeepSeek V4 Flash，并从 `DEEPSEEK_API_KEY` 环境变量读取密钥；已完成一次授权后的真实只读端到端 run；provider 空 content 单次重试见 [ADR-0018](../decisions/0018-empty-model-content-retry.md)。
...
- DeepSeek V4 Flash 已完成真实只读端到端验证；demo 修复 run 暴露 provider 空 content，已完成预算/单工具轮次/安全诊断调整和空 content 单次重试；真实写入审批、取消、超时和命令任务尚未验证。
