...
更新日期：2026-08-29（北京时间）。

**当前阶段：工程骨架、Agent 运行协议领域模型、本地 workspace 边界、读写文件工具、工具注册表、HTTP run API、SSE 事件流、mock runner、模型适配边界、响应解析器、多轮 Agent loop、运行预算、取消 API、工具超时、后台任务生命周期、审批策略、完整 approve/reject/resume 工作流、diff/变更展示、前端工具卡片/命令输出展示、Codex-like 工作台 UI 投影层、可调审查/文件侧栏和 Codex-like 视觉降噪、Frontend Design Premium 语义叙事优化、gpt-taste 审批/Markdown/审查滚动修正、Codex-like 视觉 polish、前端 SSE 消息流式显示、OpenAI-compatible DeepSeek V4 Flash 适配器、原生 tool calling 协议和 workspace 命令执行工具已建立；DeepSeek V4 Flash 原生 tool calling 真实只读端到端 run 和真实修复闭环已验证；真实模型命令 run 已通过 DeepSeek V4 Flash native demo 与 deepseek-chat 强指令 demo 验证；旧 JSON content 协议下 V4 Flash 自主/半自主 demo 未通过。**
...
- OpenAI-compatible 模型适配器已建立：默认仍为 mock，可通过配置切换到 DeepSeek V4 Flash，并从 `DEEPSEEK_API_KEY` 环境变量读取密钥；已完成真实只读端到端 run、原生 tool calling 真实只读 run 和原生 tool calling 真实修复闭环；provider 空 content 带提醒单次重试见 [ADR-0018](../decisions/0018-empty-model-content-retry.md)；原生 tool calling 协议见 [ADR-0023](../decisions/0023-native-tool-calling-protocol.md)。
...
- 增强并重构前端工作台：左侧 project/run history，中间 chat timeline，右侧 Inspector；命令审批、执行状态、stdout/stderr、exit code 和 duration 可在录屏中通过工具卡与审查详情呈现；右侧 Inspector 已精简为审查/文件入口并支持收起/展开与拖拽尺寸调整；底部 command/terminal 常驻栏已移除；前端已增加 Codex-like 语义叙事层，隐藏空工具调用消息和 run 元数据噪音，以轻量动作行、变更摘要卡和右侧 diff/file review 呈现真实工作流；审批卡按“申请写入/修改具体文件”表达，assistant 最终输出支持安全 Markdown 渲染，Return 可发送且成功创建 run 后清空输入框；已修复审批成功后 pending approval 被 SSE 刷新导致的 `null.name` 前端错误，并加入基于 SSE 完整消息事件的前端流式 reveal。
...
| 真实模型任务及回归测试 | 进行中 | [backend/src/main/java/com/zhumeiyuan/codingagent/agent/model/OpenAiCompatibleModelClient.java](../backend/src/main/java/com/zhumeiyuan/codingagent/agent/model/OpenAiCompatibleModelClient.java)、[workspaces/demo](../workspaces/demo) | [MODELAPI-001](VERIFICATION.md)、[REALMODEL-001](VERIFICATION.md)、[REALMODEL-002](VERIFICATION.md)、[DEMO-001](VERIFICATION.md)、[REALDEMO-001](VERIFICATION.md)、[REALDEMO-002](VERIFICATION.md)、[MODELAPI-005](VERIFICATION.md) | DeepSeek V4 Flash 原生 tool calling 真实只读 run 和真实修复闭环已验证；OpenAI-compatible 原生 tool calling 已通过替身 HTTP 单元验证；deepseek-chat 强指令 demo 也已完成真实写入审批和命令验证；旧 JSON content 协议下 DeepSeek V4 Flash 自主/半自主 demo 未稳定通过 |
| 提交说明、视频与面试材料 | 未开始 | 尚无 | 尚无 | 依据真实能力编写 |

## 下一步

1. 为录屏选择主模型：优先使用已跑通完整闭环的 DeepSeek V4 Flash native tool calling；若录屏现场不稳，可降级到已验证过的 `deepseek-chat` 强指令链路。
2. 补充命令进程树级取消或明确演示命令范围，避免长时间命令残留。
3. 补一次完整真实模型前端终态录屏验收，确认 Approve/Reject 后 diff、命令证据和最终总结在新 Codex-like 语义叙事 UI 中连贯展示。
4. 确认公开仓库的账户、名称及可公开文件；首次公开推送前复核题目 PDF、日志、密钥和演示材料。

## 风险与待定项

- DeepSeek V4 Flash 在旧 JSON content 协议下自主/半自主 demo 修复未稳定通过；切换到原生 tool calling 后，真实只读 run 和真实修复闭环均已通过。deepseek-chat 强指令 demo 也已完成真实写入审批、命令审批、unittest 验证和最终总结。
...
- 当前有已验证的 mock run 前后端闭环、写入/编辑工具、命令执行工具、多轮 loop、取消、工具超时、审批拦截策略、完整 approve/reject/resume、diff 元数据/展示构建、工具卡片/命令输出 UI、OpenAI-compatible 模型适配器，一次 DeepSeek V4 Flash 原生 tool calling 真实只读端到端 run，以及一次 DeepSeek V4 Flash 原生工具真实修复闭环；但没有完整进程树级取消。
